...
function as(x, xml, mxchrs) {
    ...
    for (ln = 0; ln < arr.length; ln++) {
        ...
        s = '<tr><td align=center' + sbg + ' style="color:#0055ee;border-right:1px solid #e0e0e0;">';
        s += (ln + 1) + '.</td><td' + sbg + '>';
        var ths = arr[ln];
        if (mxchrs > 0) if (ths.length > mxchrs) for (i = mxchrs - 1; i > 0; i--) {
            if (ths.charAt(i) == ' ') {
        ...
        if (nInN != 0) {
            ...
            if (sright == null) {
                ...
                if (nInN == 0) s += procpures(ths, xml); else {
...
function procnts(ths, nInN, off, xml) {
    var s = "";
    if (off == -1) s += '<font color=#008800>' + ths + '</font>'; else {
        off += (nInN == 1) ? 6 : 2;
...
function nstd() {
    var off = navigator.userAgent.indexOf("MSIE");
    if (off >= 0) {
        var s = navigator.userAgent.substring(off + 4);
        off = s.indexOf('.');
        if (off > 0) nstd_ie = parseInt(s.substring(0, off)); else nstd_ie = 7;
    }
    ...
    for (nsi = 0; nsi < nstd_tag.length; nsi++) {
        ...
        if (item != null) {
            ...
            if (fclrs != "") {
                ...
                if (off == -1) clrHi = clrLo = round_cls; else {
        ...
        if (nstd_clsname == 'round') gwround(x, bLeft, bRight, clrHi, clrLo, clrLine, lineThick, fillAngle, nstd_width, nstd_height, arcsz); else gwroundtop(x, bLeft, bRight, clrHi, clrLo, clrLine, lineThick, fillAngle, nstd_width, nstd_height);
    }
}
function gwroundtop(x, bLeft, bRight, clr1, clr2, clrLine, lineThick, fillAngle, w, h) {
    ...
    if (nstd_ie > 0) {
        ...
        if (bLeft)  rt += '-moz-border-radius-topleft:9px;';
        if (bRight) rt += '-moz-border-radius-topright:9px;';
        rt += 'background:#dddddd; background-image:-moz-linear-gradient(top, ' + clr1 + ', ' + clr2 + ');">';
        rt += s1 + '</div>';
        rt += '<div style="position:relative;width:100%;height:' + (h - 29) + 'px; top:-1px;border:' + lineThick + 'px solid ' + clrLine + ';';
        if (bRight == false) rt += '-moz-border-radius-bottomright:9px;';
        rt += 'background:#eeeeee; background-image:-moz-linear-gradient(' + (parseInt(fillAngle) + 90) + 'deg, ' + clr1 + ', ' + clr2 + ');">' + s2 + '</div>';
    }
    x.innerHTML = rt;
}
function gwround(x, bLeft, bRight, clr1, clr2, clrLine, lineThick, fillAngle, w, h, arcsz) {
    ...
    if (nstd_ie > 0) {
        ...
        if (lineThick > 0) ss = 'strokecolor="' + clrLine + '" strokeweight="' + lineThick + 'pt"';
        ss += ' fillcolor="' + clr1 + '"';
        if (bLeft == false && bRight == false) rt = '<v:Rect ' + ss + ' style="position:relative;left=-1;top=-1;width:' + w + ';height:' + h + ';">'; else rt = '<v:RoundRect ' + ss + ' style="position:relative;width:' + w + ';height:' + h + ';" arcsize="' + arcsz + '">';
        rt += '<v:fill angle=' + fillAngle + ' type="gradient" color2="' + clr2 + '"/>';
        rt += x.innerHTML;
        if (bLeft == false && bRight == false) rt += '</v:Rect>'; else rt += '</v:RoundRect>';
    } else {
        rt = '<div style="position:relative;width:100%;height:100%; border:' + lineThick + 'px solid ' + clrLine + ';';
        if (bLeft && bRight) rt += '-moz-border-radius:9px;'; else if (bLeft) rt += '-moz-border-radius-topleft:9px;-moz-border-radius-bottomright:9px;'; else if (bRight) rt += '-moz-border-radius-topright:9px;-moz-border-radius-bottomleft:9px;';
        rt += 'background:#eeeeee; background-image:-moz-linear-gradient(' + (parseInt(fillAngle) + 90) + 'deg, ' + clr1 + ', ' + clr2 + ');">';
        rt += x.innerHTML + '</div>';
    }
    x.innerHTML = rt;
}
