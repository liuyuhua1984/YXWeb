...
public class PageTool2 {
	...
	private String getLeftTools2() {
		...
		if (currentPage == 1) {
			...
			leftString = new StringBuffer("<a href='javascript:;' onclick='getPage(1);'><font class='page_up_down'>« 首页</font></a><a href='javascript:;' onclick='getPage(" + (getCurrentPage() - 1) + ");'><font class='page_up_down'>上一页</font></a>");
		}
		return leftString.toString();
	}
	...
	private String getRightTools2() {
		...
		if (currentPage == totalPageCount || totalPageCount == 0) {
			...
			rightString = new StringBuffer("<a href='javascript:;' onclick='getPage(" + (getCurrentPage() + 1) + ");'><font class='page_up_down'>下一页</font></a><a href='javascript:;' onclick='getPage(" + (getTotalPageCount()) + ");'><font class='page_up_down'>尾页 »</font></a>");
		}
		return rightString.toString();
	}
		...
		if (totalPageCount <= lineCursor) {
			for (int i = 1; i < this.totalPageCount + 1; i++) {
				if (i == this.currentPage) {
					...
					centerString.append("<a href='javascript:;' onclick='getPage(" + i + ");'>" + i + "</a>");
				}
			}
		} else {
			int s = currentPage - cursor;
