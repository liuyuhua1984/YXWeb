...
import com.gamecenter.api.controller.BaseApiController;
...
public class ApiServiceImpl implements ApiService {
	...
	public int recharge(String appId,String pssport, String billno, double money, int time, int sid, String flag) {
		...
		String res = ""; // 发货结果
		try {
			OpGameworld opGameworld = worldService.getWoldByAppIdAndWorldId(appId, ""+sid);
			if (opGameworld != null) {
...
//			con.init(BaseApiController.gameip, Integer.parseInt(BaseApiController.gameport));
			GmRechargeProtocolRequest recharge = new GmRechargeProtocolRequest();
...
//			}
			...
			recharge.setMoney(""+money); // 金币
			recharge.setServerId(""+sid);
			...
			GmRechargeHttpProtocol resp = (GmRechargeHttpProtocol)PlatformToServerConnection.sendPlatformToServer(opGameworld.getIp(), opGameworld.getServerurl(), recharge);
		        res = resp.result;
